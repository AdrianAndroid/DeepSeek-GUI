...
DeepSeek GUI 不是在 Renderer 里直接调模型。它的主链路是：

```mermaid
flowchart LR
  UI["Renderer: React + Zustand"]
  Preload["Preload: window.dsGui"]
...
- 暴露 `window.dsGui.runtimeRequest()`。
- 暴露 `window.dsGui.startSse()` / `stopSse()`。
...
```text
%APPDATA%\DeepSeek GUI\logs
%USERPROFILE%\.deepseekgui\kun
```
